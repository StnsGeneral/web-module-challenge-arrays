...
function getFlavorByIndex(getFlavor, x) {
    return getFlavor[x]
}
...
function removeFlavorByName(removeFlavor, item) {
    let x = removeFlavor.indexOf(item)
    removeFlavor.splice(x, 1)
    return removeFlavor
}
...
function getAverageWordLength(arrayLength) {
    let x = 0
    arrayLength.forEach(item => {
        x += item.split(" ").length
    });
    return x / arrayLength.length
}
